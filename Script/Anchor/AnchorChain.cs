...
public class AnchorChain : MonoBehaviour
{
    ...
    private void TickExtend()
    {
        ...
        if (TryHitPlatform(nextLength, out RaycastHit2D hit))
        {
            ...
            UpdateVisuals();
            owner?.StartAnchorPull(attachPoint, hit.collider);
            return;
        }
    ...
    private bool IsOwnerCollider(Collider2D col)
    {
        ...
        if (owner.cap != null && col == owner.cap)
            return true;

        return col.attachedRigidbody != null && col.attachedRigidbody == owner.rb;
    }
}
